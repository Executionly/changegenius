...
function build90DayPlan(
  ...
  plan.push(
    "Days 1–30: Run individual debrief sessions with each team member. Share role profiles and ADAPTS strengths so each person understands their contribution to the system.",
  );

  if (criticalStages.length > 0) {
    plan.push(
      `Days 15–45: Focus team sessions on your lowest-scoring stage — "${criticalStages[0]}". Run a structured dialogue session to surface what is blocking this capability.`,
    );
  } else {
    plan.push(
      "Days 15–45: Facilitate a Team Change Map™ session using role distribution data to assign stage ownership across the team.",
    );
  }

  if (missingRoles.length > 0) {
    plan.push(
      `Days 30–60: Your team is missing the ${missingRoles.join(" and ")} role(s). Consider bringing in external support or developing this capability through targeted coaching.`,
    );
  } else {
    plan.push(
      "Days 30–60: Begin the Weekly Change Pulse™ — a short weekly check-in to track momentum, dialogue, and alignment across the team.",
    );
  }

  if (frictionPatterns.length > 0) {
    plan.push(
      "Days 60–90: Address identified friction patterns directly. Schedule a structured retrospective using the team diagnostic report to build shared accountability.",
    );
  } else {
    plan.push(
      "Days 60–90: Review progress on 30-day action plans. Identify two team-level improvement areas and assign pod ownership for the next quarter.",
    );
  }
...
export function computeTeamDiagnostic(members: MemberScore[]): TeamDiagnostic {
  ...
  const frictionPatterns = detectFrictionPatterns(
    roleDistribution,
    stageHealth,
    dominantEnergy,
    n,
  ...
  const rollout90Days = build90DayPlan(
    missingRoles,
    criticalStages,
    frictionPatterns,
  );
